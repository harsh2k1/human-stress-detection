...
def get_face_detector(modelFile=None,
    ...
    if quantized:
        if modelFile == None:
            modelFile = config_parser['PATH']['project_path'] + "/models/opencv_face_detector_uint8.pb"
        if configFile == None:
            configFile = config_parser['PATH']['project_path'] + "/models/opencv_face_detector.pbtxt"
        model = cv2.dnn.readNetFromTensorflow(modelFile, configFile)
        
    else:
        if modelFile == None:
            modelFile = config_parser['PATH']['project_path'] + "/models/res10_300x300_ssd_iter_140000.caffemodel"
        if configFile == None:
            configFile = config_parser['PATH']['project_path'] + "/models/deploy.prototxt"
